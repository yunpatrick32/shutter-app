...
async function handleSignIn(user){ currentUser=user; const {data}=await supabase.from('profiles').select('*').eq('user_id',user.id).maybeSingle(); userProfile=data?toCreator(data):null; updateProfileBtn(); updateNotifBadge(); if(!userProfile){ setTimeout(()=>{ closeLoginModal(); openJoin(); },500); } else { closeLoginModal(); openMyProfile(); } }
...
function createPinEl(creator){ const meta=TAG_META[creator.primaryTag]; const el=document.createElement('div'); el.className='shutter-pin'; el.dataset.id=creator.id; const av=creator.schedule[0]; const photoStyle=creator.avatarUrl?`background-image:url(${creator.avatarUrl});background-size:cover;background-position:center;`:'';
  ...
  el.innerHTML=`<div class="pin-ring" style="--ring:${meta.color};--bg:${meta.bg};${photoStyle}">${initialsHtml}<span class="pin-avail ${av?'avail-yes':'avail-no'}"></span></div><span class="pin-label" style="color:${meta.color}">${creator.primaryTag}</span>`; el.addEventListener('click',e=>{e.stopPropagation();openCard(creator);}); return el; }
