...
async function fetchAndRender(){ if(fetchAndRender._running)return; fetchAndRender._running=true; const {data,error}=await supabase.from('profiles').select('*').eq('is_live',true); fetchAndRender._running=false; if(error){console.error('Supabase error:',error);return;} creators=(data||[]).map(toCreator); const now=new Date(); // Auto-expire available_now client-side and update DB for own profile creators.forEach(c=>{if(c.availableNow&&c.availableUntil&&new Date(c.availableUntil)<=now)c.availableNow=false;}); if(userProfile){const me=creators.find(c=>c.id===userProfile.id);if(me&&!me.availableNow&&userProfile.availableNow){supabase.from('profiles').update({available_now:false,available_until:null}).eq('id',userProfile.id).catch(()=>{});userProfile.availableNow=false;userProfile.availableUntil=null;}} const filtered=activeFilter==='all'?creators:creators.filter(c=>c.tags.includes(activeFilter)); renderMarkers(filtered); }
