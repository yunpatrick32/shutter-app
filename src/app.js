...
function checkLocateButton(){
  ...
  setLocateButtonVisibility(!anyOpen);
}
...
map.on('click',()=>{ if(_suppressMapClose)return; closeCard(); });
...
async function fetchAndOpenMessages(){
  ...
  setLocateButtonVisibility(false);
...
function closeMessages(){ document.getElementById('messages-panel').classList.remove('open'); setLocateButtonVisibility(true); }
...
function openJoin(){ closeAllPanels(); document.querySelectorAll('#jn-name,#jn-location,#jn-bio,#jn-gear,#jn-portfolio-url,#jn-instagram').forEach(el=>el.value=''); const termsBox=document.getElementById('jn-terms'); if(termsBox)termsBox.checked=false; const jnImg=document.getElementById('jn-avatar-img'); if(jnImg){jnImg.src='';jnImg.style.display='none';} const jnHint=document.getElementById('jn-avatar-hint'); if(jnHint)jnHint.style.display='flex'; const jnInput=document.getElementById('jn-avatar-input'); if(jnInput)jnInput.value=''; document.querySelectorAll('.spec-check').forEach(cb=>cb.checked=false); updateJnPrimaryTag(); document.getElementById('jn-half').value=''; document.getElementById('jn-full').value=''; const sr=document.getElementById('jn-show-rates'); if(sr){sr.checked=false;updateJnRatesToggle();} if(currentUser){ const meta=currentUser.user_metadata; const name=meta?.full_name||meta?.name||''; if(name)document.getElementById('jn-name').value=name; } jnPortfolioFiles=[];renderJnPortfolioGrid(); document.getElementById('join-panel').classList.add('open'); document.getElementById('overlay').classList.add('active'); setLocateButtonVisibility(false); }
function closeJoin(){ document.getElementById('join-panel').classList.remove('open'); document.getElementById('overlay').classList.remove('active'); setLocateButtonVisibility(true); }
function updateJnPrimaryTag(){
  const checked=[...document.querySelectorAll('.spec-check:checked')].map(cb=>cb.value);
  const wrap=document.getElementById('jn-primary-wrap');
  const sel=document.getElementById('jn-primary-tag');
  if(!wrap||!sel)return;
  if(!checked.length){wrap.style.display='none';return;}
  wrap.style.display='block';
  const cur=sel.value;
  sel.innerHTML=checked.map(t=>{const m=TAG_META[t]||{};return`<option value="${t}"${t===cur?' selected':''}>${m.label||t}</option>`;}).join('');
  if(!checked.includes(cur))sel.value=checked[0];
}
document.querySelectorAll('.spec-check').forEach(cb=>cb.addEventListener('change',updateJnPrimaryTag));
function renderJnPortfolioGrid(){
  const grid=document.getElementById('jn-portfolio-grid');
  if(!grid)return;
  grid.innerHTML='';
  for(let i=0;i<6;i++){
    const file=jnPortfolioFiles[i]||null;
    const cell=document.createElement('div');
    cell.style.cssText='aspect-ratio:1;border-radius:10px;overflow:hidden;position:relative;background:#111827;border:1.5px dashed #374151;';
    if(file){
      const url=URL.createObjectURL(file);
      cell.style.border='none';
      cell.innerHTML=`<img src="${url}" style="width:100%;height:100%;object-fit:cover;"/><button onclick="removeJnPortfolioPhoto(${i})" style="position:absolute;top:4px;right:4px;width:18px;height:18px;border-radius:50%;background:rgba(17,24,39,.85);border:none;color:#fff;font-size:.6rem;cursor:pointer;display:flex;align-items:center;justify-content:center;">✕</button>`;
    }else{
      const label=document.createElement('label');
      label.style.cssText='position:absolute;inset:0;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#4b5563;font-size:1.4rem;';
      const input=document.createElement('input');
      input.type='file';input.accept='image/*';input.style.display='none';
      input.addEventListener('change',e=>{
        const f=e.target.files[0];if(!f)return;
        if(jnPortfolioFiles.filter(Boolean).length>=6){showToast('Max 6 photos','#ef4444');return;}
        jnPortfolioFiles[i]=f;renderJnPortfolioGrid();
      });
      label.textContent='+';label.appendChild(input);cell.appendChild(label);
    }
...
// ─── SPECIALTIES ─────────────────────────────────────────────────────────────
function renderMpSpecialtiesGrid(){
  const grid=document.getElementById('mp-specialties-grid');
